window.onload = function(){
	...
	function addProduct(){
		...
		if(parseInt(pid) == 1){
			...
			if(q !== undefined && verifyQuantity(q)){
				 ...
				 request.open("POST", 'add_cart.php');
				...
				request.send("productId="+pid+"&quantity="+q+"&price="+cart.p1.p);
			}
			else{
	...
	document.getElementById('calculate-cart')?document.getElementById('calculate-cart').addEventListener('click', function(){
		...
		if(cart.p2.q < 1 && cart.p1.q < 1){
			...
			if(cart.p2.q > 0){
				...
				total += dollars;
				checkoutContent += '<div class="row"><span>Product Two</span><span>'+cart.p2.q+'</span><span>$'+dollars.toFixed(2)+'</span></div>';
			}
			if(cart.insure){
	...
	*/
	...
	//xmlhttprequest for ie 6 and below
